...
const AdminHeroModel = require("../../aModel/aSetting/bAdminHeroModel")
...
exports.adminHeroController = (Model= AdminHeroModel, Label= 'Admin Hero', Cache= 'adminHeroController') => {
	return {
		// List Controller
		...
		create: catchAsyncError(async (request, response, next) => {
			...
			// Invalidate Cache
			await invalidateCache(Model, nodeCache, [`${Cache}List`, 'homePageControllerAdminRetrieve'], `${Cache}Retrieve`, "Created")
		...
		update: catchAsyncError(async (request, response, next) => {
			...
			// Invalidate Cache
			await invalidateCache(Model, nodeCache, [`${Cache}List`, 'homePageControllerAdminRetrieve'], `${Cache}Retrieve`, "Updated")
		...
		delete: catchAsyncError(async (request, response, next) => {
			...
			// Invalidate Cache
			await invalidateCache(Model, nodeCache, [`${Cache}List`, 'homePageControllerAdminRetrieve'], `${Cache}Retrieve`, "Deleted")
