...
exports.blogCardPageController = (Label= 'Blog Card Page') => {
	return {
		// List Controller
		list: catchAsyncError(async (request, response, next) => {
      let list = await BlogModel.findOne().sort({ _id: -1 }).populate({
        path: 'cBlogCards',
        model: BlogCardModel,
      });
    ...
    // Retrieve Controller
		retrieve: catchAsyncError(async (request, response, next) => {
			// Retrieve
			let retrieve = await BlogCardModel.findById(request.params.id)
