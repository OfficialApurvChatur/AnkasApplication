...
exports.eventCardPageController = (Label= 'Event Card Page') => {
	return {
		// List Controller
		list: catchAsyncError(async (request, response, next) => {
      let list = await EventModel.findOne().sort({ _id: -1 }).populate({
        path: 'cEventCards',
    ...
    // Retrieve Controller
		retrieve: catchAsyncError(async (request, response, next) => {
			// Retrieve
			let retrieve = await EventCardModel.findById(request.params.id)
